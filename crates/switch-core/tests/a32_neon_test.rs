...
use a32::{cpu as new_cpu, r, BASE, HALT, SCRATCH};
...
fn with_quads(a: [f32; 4], b: [f32; 4], ops: &[u32]) -> Cpu {
    ...
    let mut bytes = Vec::new();
    for insn in ops.iter().chain(std::iter::once(&HALT)) {
        bytes.extend_from_slice(&insn.to_le_bytes());
    }
    cpu.mem.map(BASE, &bytes).unwrap();
...
fn the_immediate_move_expands_its_pattern_across_the_register() {
    ...
    let mut bytes = Vec::new();
    for insn in [0xF280_0050u32, HALT] {
        bytes.extend_from_slice(&insn.to_le_bytes());
    }
    cpu.mem.map(BASE, &bytes).unwrap();
    cpu.run(2).unwrap();
    ...
    cpu.set_vreg(0, u128::MAX);
    let mut bytes = Vec::new();
    for insn in [0xF387_0E1Fu32, HALT] {
        bytes.extend_from_slice(&insn.to_le_bytes());
    }
    cpu.mem.map(BASE, &bytes).unwrap();
    cpu.run(2).unwrap();
...
fn the_integer_add_wraps_within_each_lane() {
    ...
    let mut bytes = Vec::new();
    for insn in [0xF220_4842u32, HALT] {
        bytes.extend_from_slice(&insn.to_le_bytes());
    }
    cpu.mem.map(BASE, &bytes).unwrap();
    cpu.run(2).unwrap();
...
fn the_bitwise_operations_are_selected_by_the_size_field() {
    ...
    let mut bytes = Vec::new();
    for insn in [0xF220_4152u32, 0xF200_4152, HALT] {
        bytes.extend_from_slice(&insn.to_le_bytes());
    }
    cpu.mem.map(BASE, &bytes).unwrap();
    cpu.run(1).unwrap();
...
fn bit_select_takes_its_mask_from_the_destination() {
    ...
    let mut bytes = Vec::new();
    for insn in [0xF310_4152u32, HALT] {
        bytes.extend_from_slice(&insn.to_le_bytes());
    }
    cpu.mem.map(BASE, &bytes).unwrap();
    cpu.run(2).unwrap();
...
fn the_multiply_by_element_broadcasts_one_lane() {
    ...
    let mut bytes = Vec::new();
    for insn in [0xF3A0_4142u32, HALT] {
        bytes.extend_from_slice(&insn.to_le_bytes());
    }
    cpu.mem.map(BASE, &bytes).unwrap();
    cpu.run(2).unwrap();
    ...
    cpu.set_vreg(1, quad([10.0, 100.0, 0.0, 0.0]));
    let mut bytes = Vec::new();
    for insn in [0xF3A0_4962u32, HALT] {
        bytes.extend_from_slice(&insn.to_le_bytes());
    }
    cpu.mem.map(BASE, &bytes).unwrap();
    cpu.run(2).unwrap();
...
fn ext_slides_a_window_across_the_register_pair() {
    ...
    let mut bytes = Vec::new();
    for insn in [0xF2B0_4442u32, HALT] {
        bytes.extend_from_slice(&insn.to_le_bytes());
    }
    cpu.mem.map(BASE, &bytes).unwrap();
    cpu.run(2).unwrap();
...
fn the_table_lookup_gathers_bytes_and_zeroes_the_misses() {
    ...
    let mut bytes = Vec::new();
    for insn in [0xF3B0_4902u32, HALT] {
        bytes.extend_from_slice(&insn.to_le_bytes());
    }
    cpu.mem.map(BASE, &bytes).unwrap();
    cpu.run(2).unwrap();
...
fn dup_broadcasts_one_lane_to_every_lane() {
    ...
    let mut bytes = Vec::new();
    for insn in [0xF3BC_4C40u32, HALT] {
        bytes.extend_from_slice(&insn.to_le_bytes());
    }
    cpu.mem.map(BASE, &bytes).unwrap();
    cpu.run(2).unwrap();
...
fn the_two_register_miscellaneous_operations() {
    ...
    cpu.set_vreg(0, 0x0F0E_0D0C_0B0A_0908_0706_0504_0302_0100);
    let mut bytes = Vec::new();
    for insn in [0xF3B0_40C0u32, HALT] {
        bytes.extend_from_slice(&insn.to_le_bytes());
    }
    cpu.mem.map(BASE, &bytes).unwrap();
    cpu.run(2).unwrap();
    ...
    let mut bytes = Vec::new();
    for insn in [0xF3B0_4500u32, HALT] {
        bytes.extend_from_slice(&insn.to_le_bytes());
    }
    cpu.mem.map(BASE, &bytes).unwrap();
    cpu.run(2).unwrap();
...
fn the_vector_loads_and_stores_move_two_registers_and_advance_the_base() {
    ...
    code.push(0xE3A0_1A08); // mov r1, #0x8000
    code.push(0xE1A0_0001); // mov r0, r1
    code.push(0xF401_2A8D); // vst1.32 {d2, d3}, [r1]!
    code.push(0xF420_0A8D); // vld1.32 {d0, d1}, [r0]!
    let mut bytes = Vec::new();
    for insn in code.iter().chain(std::iter::once(&HALT)) {
        bytes.extend_from_slice(&insn.to_le_bytes());
    }
...
fn vsel_reads_the_condition_flags_it_names() {
    ...
    let mut cpu = new_cpu();
    let mut code: Vec<u32> = Vec::new();
    code.push(0xE3A0_0001); // mov r0, #1  -> sets no flags
    code.push(0xE350_0002); // cmp r0, #2  -> 1 < 2, so GT is false
    code.push(0xFE30_1A20); // vselgt.f32 s2, s0, s1
    let mut bytes = Vec::new();
