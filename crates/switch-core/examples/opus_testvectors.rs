...
fn main() {
    ...
    for name in &names {
        ...
        for channels in [2usize, 1] {
            ...
            while at + 8 <= bits.len() {
                let len = u32::from_be_bytes([bits[at], bits[at + 1], bits[at + 2], bits[at + 3]])
                    as usize;
                let expected_range =
            ...
            match (&error, mismatch) {
                ...
                (None, None) => {
                    println!(
                        "{name}{suffix} ok   {packet} packets, {} samples -> {path}",
                        out.len() / (2 * channels)
                    )
                }
            }
    ...
    println!(
        "\ndecoded {audio:.1} s of audio in {elapsed:.2} s ({:.0}x real time)",
        audio / elapsed
    );
