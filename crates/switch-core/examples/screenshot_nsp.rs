...
fn main() {
        ...
        |cpu, done| {
            ...
            if let Some(w) = watch {
                if !seen_nonzero
                    ...
                    && (0..0x1000u32)
                        .step_by(4)
                        .any(|k| cpu.mem.read_u32(w + k).unwrap_or(0) != 0)
                {
                ...
                if done % (1 << 22) == 0 {
                    ...
                    for v in 0..3u32 {
                        ...
                        for c in 0..3u32 {
                            let _ = cpu
                                .mem
                                .write_u32(at + c * 4, POS[v as usize][c as usize].to_bits());
                        }
                        for c in 0..2u32 {
    ...
    if let Ok(v) = env::var("SCAN_MEM") {
        ...
        if let Some(st) = run {
            spans.push((at + st, len - st));
        }
        println!("  {} non-zero spans in {at:#x}+{len:#x}", spans.len());
        for (a, l) in spans.iter().take(40) {
            println!("    {a:#x} .. +{l:#x}");
        }
    }
