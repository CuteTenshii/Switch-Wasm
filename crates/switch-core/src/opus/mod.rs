...
fn parse_toc(byte: u8, fs: u32) -> Toc {
    ...
    Toc { mode, bandwidth, frame_size, channels: if byte & 0x4 != 0 { 2 } else { 1 } }
}
...
impl Decoder {
    ...
    pub fn decode_float(&mut self, packet: Option<&[u8]>, pcm: &mut [f32], frame_size: usize) -> Result<usize> {
        self.decode_native(packet, pcm, frame_size, false).map(|(samples, _)| samples)
    }
    ...
    fn decode_native(
        ...
        for &(offset, size) in &frames {
            let got = self.decode_frame(Some(&data[offset..offset + size]), pcm, done, frame_size - done)?;
            done += got;
        }
        self.last_packet_duration = done;
    ...
    pub fn decode(&mut self, packet: Option<&[u8]>, pcm: &mut [i16], frame_size: usize) -> Result<usize> {
    ...
    fn decode_frame(&mut self, data: Option<&[u8]>, pcm: &mut [f32], at: usize, room: usize) -> Result<usize> {
        ...
        let (mode, bandwidth, mut audiosize) = match data {
            ...
            None => {
                let mode = if self.prev_redundancy { Some(Mode::CeltOnly) } else { self.prev_mode };
        ...
        if mode != Mode::CeltOnly {
            ...
            while decoded < frame_size {
                ...
                let got = match dec.as_mut() {
                    Some(dec) => self.silk.decode(&control, false, first, Some(dec), &mut silk_pcm[decoded * self.channels..]),
                    None => self.silk.decode(&control, true, first, None, &mut silk_pcm[decoded * self.channels..]),
                };
                match got {
                    Ok(n) => decoded += n,
                    Err(_) => {
                        // A concealment failure is not fatal; silence is.
                        for v in silk_pcm[decoded * self.channels..frame_size * self.channels].iter_mut() {
                            *v = 0;
                        }
                        decoded = frame_size;
                    }
        ...
        if mode != Mode::CeltOnly {
            if let Some(dec) = dec.as_mut() {
                if dec.tell() + 17 + 20 * i32::from(mode == Mode::Hybrid) <= 8 * len as i32 {
                    redundancy = if mode == Mode::Hybrid { dec.decode_bit_logp(12) } else { true };
                    if redundancy {
        ...
        if redundancy && celt_to_silk {
            ...
            self.celt.decode(Some(&mut rdec), redundancy_bytes, &mut redundant_audio, f5);
        ...
        if redundancy && !celt_to_silk {
            ...
            fade_in_over(&redundant_audio[self.channels * f2_5..], &mut out[tail..], f2_5, self.channels, self.fs);
        }
        if redundancy && celt_to_silk && (self.prev_mode != Some(Mode::SilkOnly) || self.prev_redundancy) {
            ...
            fade_out_under(&redundant_audio[self.channels * f2_5..], &mut out[self.channels * f2_5..], f2_5, self.channels, self.fs);
        }
        if transition {
            if let Some(prev) = pcm_transition {
                if audiosize >= f5 {
                    out[..self.channels * f2_5].copy_from_slice(&prev[..self.channels * f2_5]);
                    fade_out_under(&prev[self.channels * f2_5..], &mut out[self.channels * f2_5..], f2_5, self.channels, self.fs);
                } else {
...
impl MultiStreamDecoder {
    ...
    pub fn new(fs: u32, channels: usize, streams: usize, coupled: usize, mapping: &[u8]) -> Result<Self> {
    ...
    pub fn decode_float(&mut self, packet: Option<&[u8]>, pcm: &mut [f32], frame_size: usize) -> Result<usize> {
        ...
        for s in 0..self.streams {
            ...
            let (got, offset) = self.decoders[s].decode_native(sub, &mut buf, produced, self_delimited)?;
            at += offset;
            produced = got;

            let stride = if s < self.coupled { 2 } else { 1 };
            for half in 0..stride {
                let tag = if s < self.coupled { (2 * s + half) as u8 } else { (self.coupled + s) as u8 };
...
mod tests {
    ...
    const CELT_PACKETS: [&[u8]; 3] = [
        &[],
        &[],
        &[],
    ];
    ...
    const SILK_PACKETS: [&[u8]; 3] = [
        &[0x48, 0x83, 0x9c, 0x2d, 0xb5, 0xa7, 0xe7, 0xf6, 0x4c, 0x00, 0x00, 0x1f, 0x4e, 0xce, 0xfe, 0x00, 0x94, 0xa3, 0x57, 0x91, 0x64, 0x45, 0x0a, 0xaf, 0x36, 0xbc, 0xff, 0x7f, 0x2c, 0x37, 0xa9, 0xc6, 0x90, 0x34, 0xb4, 0xb7, 0x97, 0xbb, 0x5b, 0x76, 0x80, 0x67, 0x49, 0xe2, 0xfd, 0x88, 0x38, 0x2b, 0x56, 0x7a, 0xf0, 0xa6, 0x87, 0x2b, 0xc3, 0x19, 0xd8, 0x6f, 0x1e, 0x6d, 0x45, 0x8d, 0x8d, 0xb2, 0x99, 0xa2, 0x06, 0xf9, 0xbe, 0x05, 0x9d, 0x80],
        &[0x48, 0xaf, 0x97, 0x1d, 0x57, 0xbb, 0xb6, 0x2c, 0xfe, 0xe5, 0x8d, 0x5f, 0x16, 0xa4, 0x76, 0xd4, 0x96, 0x22, 0xe4, 0x4f, 0xc2, 0xa9, 0x0a, 0x5b, 0x0f, 0x91, 0x2f, 0x9d, 0x9b, 0x0a, 0x86, 0xd2, 0x79, 0xeb, 0x96, 0xdb, 0x31, 0xe3, 0x22, 0x2c, 0xca, 0x2e, 0x43, 0xcf, 0x14, 0xa8, 0xf5, 0xc3, 0xd6, 0xb1, 0x91, 0x28, 0x4a, 0xc4, 0x86, 0x36, 0x0c, 0xa7],
        &[0x48, 0xa5, 0x26, 0x7a, 0x8f, 0x85, 0xd5, 0x49, 0x03, 0xf3, 0x28, 0x5d, 0x2b, 0x5a, 0x62, 0x31, 0x58, 0xa8, 0xab, 0xf7, 0x30, 0x62, 0x2c, 0xb5, 0x97, 0x2c, 0x09, 0x84, 0xba, 0xc3, 0x6c, 0x1a, 0x9d, 0xe9, 0xfb, 0xf1, 0x76, 0xc0, 0x57, 0xeb, 0x96, 0x0a, 0xef, 0x57, 0x08, 0x90, 0x0b, 0x50, 0xbc, 0xba, 0xb4, 0xc9, 0x38, 0x33, 0x83, 0x06, 0x77, 0x9b, 0xf8],
    ];
    ...
    const HYBRID_PACKETS: [&[u8]; 3] = [
        &[],
        &[0x7c, 0x8a, 0x0f, 0x71, 0xf0, 0xd8, 0x6c, 0x69, 0x5d, 0xd9, 0x5f, 0x41, 0x37, 0x75, 0xd7, 0x5c, 0x0f, 0x2a, 0x7b, 0xf1, 0xe6, 0x2a, 0x19, 0xee, 0x41, 0x37, 0xa0, 0xf5, 0x3e, 0x73, 0x33, 0x76, 0x9d, 0x22, 0xeb, 0x52, 0x32, 0xd1, 0xc1, 0x95, 0xaf, 0xa5, 0x20, 0x75, 0xf6, 0x33, 0xf2, 0xb2, 0xc5, 0x17, 0x0e, 0xc0, 0xa3, 0xe4, 0x9c, 0x12, 0xcb, 0x90, 0x34, 0xb3, 0x76, 0x19, 0x9e, 0xae, 0xa7, 0xa1, 0x11, 0x85, 0xf8, 0xec, 0x11, 0xe2, 0xda, 0x0b, 0x83, 0xbe, 0xc6, 0xdd, 0x15, 0xb0, 0x52, 0x45, 0x30, 0x3e, 0x51, 0x08, 0xb0, 0xb9, 0xe1, 0x50, 0x99, 0xd7, 0xcc, 0x1a, 0x44, 0xec, 0x69, 0x45, 0x4c, 0x11, 0xbe, 0x72, 0x2a, 0x34, 0xa5, 0x4a, 0x8b, 0x09, 0xbe, 0x5d, 0x40, 0x9b, 0x90, 0xa7, 0xfb, 0x54],
        &[0x7c, 0x8a, 0x0f, 0x39, 0xf3, 0xdd, 0x2e, 0xa7, 0x2c, 0x83, 0x5d, 0x59, 0x61, 0xe7, 0xaf, 0xd5, 0xa4, 0xbd, 0x28, 0xe3, 0xac, 0xf2, 0xbc, 0xa8, 0x09, 0x0e, 0x98, 0xf8, 0x47, 0xdb, 0xb6, 0xfd, 0x86, 0xe9, 0x2d, 0x38, 0x63, 0x0f, 0xbe, 0xeb, 0x03, 0x07, 0x8a, 0xa0, 0x0e, 0xfd, 0x77, 0x2f, 0xa8, 0x44, 0x5c, 0xf7, 0xce, 0xf4, 0x00, 0xc6, 0x8e, 0x84, 0xe9, 0x5a, 0x49, 0x8c, 0xab, 0x78, 0x5e, 0xd8, 0x1c, 0xdd, 0xd0, 0x1f, 0x83, 0x3a, 0xb8, 0x86, 0x5e, 0x4a, 0xdd, 0xbb, 0x2b, 0x48, 0xa0, 0x77, 0x72, 0x14, 0x80, 0xd9, 0x6d, 0x04, 0x59, 0x61, 0x50, 0xbf, 0xc5, 0xc1, 0x50, 0xfa, 0xb9, 0xbe, 0x1a, 0x3a, 0xee, 0x22, 0xce, 0xf5, 0x64, 0xf6, 0xff, 0xa7, 0x1e, 0xd4, 0xe0, 0x10, 0x8d, 0xad],
    ];
    ...
    fn decodes_hybrid_packets_the_way_the_reference_does() {
        check(&HYBRID_PACKETS, &HYBRID_PACKETS_RANGES, &HYBRID_PACKETS_RMS, 2);
    }
    ...
    fn conceals_a_lost_packet() {
        ...
        decoder.decode_float(Some(CELT_PACKETS[0]), &mut pcm, 960).unwrap();
        ...
        assert_eq!(decoder.final_range(), 0, "a concealed frame read no symbols");
    }
    ...
    fn decodes_at_every_supported_output_rate() {
        for (rate, samples) in [(8000, 160), (12000, 240), (16000, 320), (24000, 480), (48000, 960)] {
            let mut decoder = Decoder::new(rate, 1).unwrap();
            let mut pcm = vec![0.0f32; samples];
            let got = decoder.decode_float(Some(CELT_PACKETS[0]), &mut pcm, samples).unwrap();
            assert_eq!(got, samples, "{rate} Hz decoded {got} samples, not {samples}");
        }
    ...
    fn parses_every_packet_framing() {
        ...
        assert_eq!(parse_frames(&[0x01, 1, 2, 3], false).err(), Some(Error::InvalidPacket));
    ...
    fn parses_self_delimited_framing() {
        ...
        assert_eq!(offset, 4, "the bytes after the frame belong to the next stream");
    ...
    fn refuses_a_packet_that_is_not_one() {
        ...
        assert_eq!(parse_frames(&[0x03, 0x00, 1], false).err(), Some(Error::InvalidPacket));
        // A coded length longer than what is left.
        assert_eq!(parse_frames(&[0x02, 9, 1, 2], false).err(), Some(Error::InvalidPacket));
    }
    ...
    fn multistream_maps_streams_onto_channels() {
        ...
        assert_eq!(decoder.decode_float(Some(&packet), &mut pcm, 960).unwrap(), 960);
        for i in 0..960 {
            assert_eq!(pcm[i * 4 + 1], pcm[i * 4 + 2], "both copies of stream 1 differ");
    ...
    fn survives_arbitrary_input() {
        ...
        for &original in CELT_PACKETS.iter().chain(SILK_PACKETS.iter()).chain(HYBRID_PACKETS.iter()) {
            for _ in 0..200 {
