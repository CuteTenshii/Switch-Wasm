...
use super::{AudioOut, AudrenParams, BsdSocket, Cpu, FontRegion};
use crate::Result;
...
impl Cpu {
    ...
    /// service itself.
    fn ipc_interface(&self, tls: u32, handle: u64, root: &'static str) -> String {
        if self.ipc_is_domain_request(tls) {
            let object_id = self.ipc_domain_object_id(tls);
    ...
    ///
    /// Nothing here ever signals one. Each describes something that does not
    /// happen on this console — a Bluetooth radio turning on, a save transfer
    /// finishing, a news article arriving — so a caller waiting on it is
    /// waiting for something that genuinely never comes, which is the
    /// truthful state rather than the silent one.
    fn kept_event(&mut self, purpose: &'static str, object: u64) -> u64 {
    ...
    /// each of them.
    fn zero_output_buffer(&mut self, tls: u32, index: u32) {
    ...
    fn write_output_buffer(&mut self, tls: u32, index: u32, bytes: &[u8]) -> u32 {
        let Some((addr, size)) = self.ipc_output_buffer(tls, index) else {
            return 0;
        };
    ...
    /// of 1065353216.
    fn ipc_arg_f32(&self, tls: u32, offset: u32) -> f32 {
        let data = self.ipc_request_data(tls);
    ...
    fn ipc_arg_u32(&self, tls: u32, offset: u32) -> u32 {
        let data = self.ipc_request_data(tls);
    ...
    fn ipc_arg_u8(&self, tls: u32, offset: u32) -> u8 {
        let data = self.ipc_request_data(tls);
